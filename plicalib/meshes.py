...
class TriangularMesh:
    ...
    def vertex_normals(self):
        if self._vertex_normals is None:
            self._vertex_normals = compute_vertex_normals(self.vertices, self.triangles)
        return self._vertex_normals
    
    @property
    def face_normals(self):
        if self._face_normals is None:
            self._face_normals = compute_face_normals(self.vertices, self.triangles)
        return self._face_normals
    
    @property
    def vertices_adj_matr(self):
        if self._vertices_adj_matr is None:
            self._vertices_adj_matr = get_vertices_adjacency_scipy(self.triangles)
        return self._vertices_adj_matr

    @property
    def faces_adj_matr(self):
        if self._faces_adj_matr is None:
            self._faces_adj_matr = get_faces_adjacency_scipy(self.triangles, return_edges=False)
    ...
    def cotan_weights(self):
        if self._cotan_weights is None:
            self._cotan_weights = compute_cotan_weights(self.vertices,
                                                                    self.triangles,
                                                                    return_as_tuple=True,
    ...
    def cotan_matrix(self):
        if self._cotan_matrix is None:
            if self._cotan_weights is None:
                self._cotan_matrix, self._cotan_weights = compute_cotan_matrix(self.vertices,
                                                                    self.triangles,
                                                                    with_diagonal=True,
                                                                    return_cotan_weights=True)
            else:
                self._cotan_matrix = compute_cotan_matrix(self.vertices,
                                                                    self.triangles,
    ...
    def mass_matrix(self):
        if self._mass_matrix is None:
            self._mass_matrix = compute_voronoi_mass(self.vertices, self.triangles, self.cotan_weights)
