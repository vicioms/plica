...
from plicalib.meshes import normals_misalignment_chull
...
def load_mesh_preprocess(
    ...
    taubin_filter_iterations : Optional[int] = None
):
    mesh = load_mesh_base(
        name=name,
        file=file,
    ...
    clusters, lengths, _ = mesh.cluster_connected_triangles()
    ...
    if holes_nbe is not None and holes_nbe > 0:
        with suppress_c_output():
            mfix = pymeshfix.PyTMesh()
            mfix.load_array(np.asarray(mesh.vertices), np.asarray(mesh.triangles))
            #mfix.clean()
            mfix.fill_small_boundaries(holes_nbe, refine=False)
    ...
    if merge_relative_scale is not None:
        if merge_relative_scale > 0:
            vertices = np.asarray(mesh.vertices)
            scale = (vertices.max(axis=0) - vertices.min(axis=0)).min()
            mesh = mesh.merge_close_vertices(merge_relative_scale * scale)
    ...
    if target_num_triangles is not None:
        if target_num_triangles > 0:
            mesh = mesh.filter_smooth_laplacian(number_of_iterations=taubin_filter_iterations)
    ...
    if orient_by_chull:
        ...
        misalignment_ratio = normals_misalignment_chull(vertices, vertex_normals)
        if misalignment_ratio > 0.5:
